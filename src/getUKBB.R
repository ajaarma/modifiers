...
#raw_var = "/hps/nobackup/dunham/ai-uk-can/ukb200k/filter_ukb200k/batch1/chr16/xaj/tmpFile_chr16_xaj.AC0.norm.vep.anno.sites.exon.0_01.imp.tab.bg.gz"
#chrNum = 16
#outDir = "/hps/nobackup/dunham/ai-uk-can/ukb200k/query_ukbb/batch1/chr16/xaj"
...
#geneListFile = '/nfs/research/dunham/samples/ddd/data/gene_list/intel_gene_list_chrMapSort.txt'
...
for (i in 1:dim(geneList)[1]){
    ...
    if (dim(df.gene.grep)[1]!=0) {
        ...
        if (dim(df.gene)[1] !=0){
            ...
            df.gene.clinvar.path.1 = tryCatch({
            #df.gene.clinvar.path.1 = 
                            df.gene[apply(data.frame(df.gene[,c("ANNO_SYMBOL","CLNSIG")]),1,function(x){
                                                       gene_list = x[1]
                                                       clin_sig_list = x[2]
                                                       gene_index = grep(gene_id.app,strsplit(gene_list,"[|]")[[1]])
                                                       clin_sig_strs = strsplit(clin_sig_list,"[|]")[[1]][gene_index]
            ...
            if (chrNum !="Y"){
                ...
                df.gene.clinvar.path.2 = tryCatch({
                #df.gene.clinvar.path.2 = 
                    df.gene[apply(data.frame(df.gene[,c("ANNO_SYMBOL","CLNSIGCONF")]),1,function(x){
                                                           gene_list = x[1]
                                                           clin_sig_list = x[2]
                                                           gene_index = grep(gene_id.app,strsplit(gene_list,"[|]")[[1]])
                                                           clin_sig_strs = strsplit(clin_sig_list,"[|]")[[1]][gene_index]
            ...
            #df.gene.path = unique(rbind(df.gene.vep.path,df.gene.clinvar.path.1,df.gene.clinvar.path.2,fill=TRUE))
        ...
        #
        #df.gene.vep.path.all = subset(df.gene,grepl("pathogenic$",df.gene$ANNO_CLIN_SIG))
        #df.gene.vep.path.notlpath = subset(df.gene.vep.path.all,!grepl("likely_pathogenic",df.gene.vep.path.all$ANNO_CLIN_SIG))
        #df.gene.vep.like.path = subset(df.gene, (grepl("pathogenic",df.gene$ANNO_CLIN_SIG) & 
        #                                      grepl("likely_pathogenic",df.gene$ANNO_CLIN_SIG) 
        #                                     )
        #                            )
        #df.gene.vep.path = unique(rbind(df.gene.vep.path.notlpath,df.gene.vep.like.path))
        
        #Process Clinvar ACMG
        #df.gene.clinvar.path = subset(df.gene,grepl("Pathogenic",df.gene$CLNSIG,perl=T,ignore.case=F) |
        #                                      grepl("Pathogenic",df.gene$CLNSIGCONF,perl=T,ignore.case=F) 
        #                             )

        #Combine VEP and CLinvar df
        #df.gene.path = unique(rbind(df.gene.vep.path,df.gene.clinvar.path))
        ...
        if (dim(df.gene.path)[1]!=0){
            ...
            if (dim(df.gene.count)[1]>0) {
                var_count = sum(df.gene.count$nHetsList)
                count_file = paste(outDir,'/',gene_name,'_variants_count.txt',sep="")
                cat(as.character(gene_name),'\t',var_count,'\n',file=count_file)
            }
            write.table(df.gene.path.2,sep="\t",file=out_file,col.names=T,row.names=F,quote=F)
        }
        else{
            cat('--No Variants found for:',gene_name$V1,'\n')
        }
    }else{
            cat('--Gene Doesnot  exist:',gene_name$V1,'\n')
