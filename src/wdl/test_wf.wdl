...
task annoMergeExon {
        ...
        output {
            ...
            File annot_merge_exon = "output/~{file_prefix}.annot.exon.bcf" 
...
task sortByExon {
    input {
        ...
        String bcf_docker
        String file_prefix = basename(annot_exon,".bcf")
    }    
    ...
    command <<<
        ...
        ~{file_prefix}.sorted.bcf

        mv ~{file_prefix}.sorted.bcf \ 
        ~{file_prefix}.bcf
    >>>
    
    output {
        File exon_bcf = "~{file_prefix}.bcf"
        File exon_index = "~{file_prefix}.bcf"
    }
    runtime {
        docker : bcf_docker
        dx_timeout : "48H"
...
workflow filterSnvWF {
        ...
        call sortByExon {
            input:
                exon_region = exon_region,
                annot_exon = annoMergeExon.annot_merge_exon,
                bcf_docker = bcf_docker
        }
        call mafImpactTab {
            input:
