...
#raw_var = "/hps/nobackup/dunham/ddd/filterSNV/gdd_mild/tmp_data/chr22/tmpFile_chr22.AC0.norm.vep.anno.sites.gdd.freq.mild.exon.0_01.imp.tab"
#chrNum = "chr22"
#outDir = "/hps/nobackup/dunham/ddd/filterSNV/gdd_mild/full_genes/chr22"
...
#### Extract all genes pertaining to chromosomes for filtering criteria ########

#varData = as.data.frame(varData)
varGenes = apply(as.data.frame(varData$ANNO_SYMBOL),
                 1,function(x){
                    y = strsplit(x,"[|]")[[1]]
                    return(unique(y))
                    }
...
geneNames = unique(unlist(varGenes))
geneNames.2 = geneNames[which(geneNames!="")]
chrNames = rep(unique(varData$CHROM),length(geneNames.2))

geneList = data.frame(cbind(geneNames.2,chrNames))
colnames(geneList) = c("Gene","Chromosome")

write.table(geneList,sep='\t',file=paste(outDir,".genesChrMap.txt",sep="/"),col.names=F,row.names=F,quote=F)
...
for (i in 1:dim(geneList)[1]){
    ...
    cat('--GeneName:',as.vector(gene_name),'\t',i,'\n')
    ...
    if (dim(df.gene.grep)[1] !=0) {
        ...
        if (dim(df.gene.path)[1]!=0){
            ...
            cat('--No Pathogenic Variants found for:',gene_name,'\n')
        }
    }else{
            #cat('--Gene Donot exist:',gene_name$V1,'\n')
            cat('--Gene Donot exist:',gene_name,'\n')
    }
}
