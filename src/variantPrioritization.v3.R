...
loc3_3="/hps/software/users/dunham/R_lib/4.0.3"

library("optparse", lib.loc=loc3_3)
library("plyr", lib.loc=loc3_3)
library("data.table", lib.loc=loc3_3)
library("parallel")
library("tools")
...
#                             | df$CHROM=="Y", F, T)
  ...
  df_prior <- do.call("rbind",
                      mclapply(unique(df$VAR), function(variant){
                                x <- df[df$VAR == variant,]
                                x <- x[with(x, order(biotype_rank, Effect_impact_rankings, 
                                                     Canonical_rank, -CDS_length
                                                     )
                                            ), 
                                      ]
                                x <- x[1,]
